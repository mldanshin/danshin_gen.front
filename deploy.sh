...
if [ $answer == "y" ]; then
    ...
    echo "Выполни на удалённом сервере команды! Перейди в каталог cd /var/www/danshin_gen/front. Далее y"
    read answer1
    if [ $answer1 == "y" ]; then
        echo "Выполни npm install. Далее y"
        read answer2
        if [ $answer2 == "y" ]; then
            echo "Выполни npm run build. Далее y"
            read answer3
            if [ $answer3 == "y" ]; then
